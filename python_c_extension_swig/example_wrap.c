...
extern "C" {
#endif
SWIGINTERN int Swig_var_my_var_set(PyObject *_val) {
  {
    double val;
    int res = SWIG_AsVal_double(_val, &val);
    if (!SWIG_IsOK(res)) {
      SWIG_exception_fail(SWIG_ArgError(res), "in variable '""my_var""' of type '""double""'");
    }
    my_var = (double)(val);
  }
  return 0;
...
SWIGINTERN PyObject *Swig_var_my_var_get(void) {
  PyObject *pyobj = 0;
  
  pyobj = SWIG_From_double((double)(my_var));
  return pyobj;
}
...
# endif
  ...
  SWIG_addvarlink(SWIG_globals(),(char*)"my_var",Swig_var_my_var_get, Swig_var_my_var_set);
#if PY_VERSION_HEX >= 0x03000000
  return m;
#else
  return;
